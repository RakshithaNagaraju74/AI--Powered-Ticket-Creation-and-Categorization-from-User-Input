...
app.get('/api/stats/live', async (req, res) => {
  try {
    ...
    // 1. DYNAMIC AGENT COUNT: Get from the Socket Map, not the DB
    const onlineAgentsCount = Array.from(activeUsers.values())
      .filter(u => u.role === 'agent').length;

    // 2. Fetch Ticket counts
    const ticketsToday = await Ticket.countDocuments({ created_at: { $gte: todayStart } });
    const resolvedToday = await Ticket.countDocuments({ status: 'resolved', updatedAt: { $gte: todayStart } });
    const activeTickets = await Ticket.countDocuments({ status: { $in: ['open', 'in_progress'] } });

    res.json({
      activeAgents: onlineAgentsCount, // 👈 This is now live
      ticketsToday,
      resolvedToday,
      activeTickets,
      satisfactionRate: "98%",
